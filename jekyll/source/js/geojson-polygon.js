function makeGeojsonPolygonLayer(geojson, featureName, map) {
    var layer = null;
    geojson.features.forEach(function(feature) {
        var locations = [];
        var coordinates, fillStyle, strokeStyle;
        if (feature.properties.name === featureName) {
            coordinates = feature.geometry.coordinates[0];
            locations = coordinates.map(function(coordinate) {
                return { lat: coordinate[1], lon: coordinate[0] };
            });
            
            var fillStyle = '#9AB59D';
            var strokeStyle = '#197123'
            layer = new com.modestmaps.PolygonMarker(map, locations, fillStyle, strokeStyle);
        }
    });

    if (layer === null) {
        throw "Can't find feature with this name: " + featureName;
    }
    else {
        return layer;
    }
...
// Taken from the hurricanes example that ships with modestmaps
// namespacing!
if (!com) {
    var com = { };
    if (!com.modestmaps) {
